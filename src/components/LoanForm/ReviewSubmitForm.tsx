  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      addApplication({
        ...
        notes: 'New application submitted by the customer',
        loanDetails: {
          purpose: formData.loanPurpose.replace(/_/g, ' '),
          requestedAmount: `R${formData.loanAmount.toLocaleString()}`,
          interestRate: '15.5%',
          term: formData.loanTerm.toString(),
        ...
        employmentInfo: {
          ...
          jobTitle: formData.jobTitle,
          employmentLength: `${formData.yearsEmployed} years`,
          monthlyIncome: `R${formData.monthlyIncome.toLocaleString()}`,
          paymentDate: formData.paymentDate
        },
        financialInfo: {
          bankName: formData.bankName,
          ...
          existingLoans: formData.existingLoans ? [
            {
              type: 'Personal',
              amount: `R${formData.existingLoanAmount?.toLocaleString() || '0'}`,
              remaining: `R${formData.existingLoanAmount?.toLocaleString() || '0'}`
            }
          ] : [],
          monthlyDebt: `R${formData.monthlyDebt?.toLocaleString() || '0'}`
        },
        email: formData.email,
        phone: formData.phone,
        idNumber: formData.idNumber || '',
