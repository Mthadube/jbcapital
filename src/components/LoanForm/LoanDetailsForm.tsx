...
const LoanDetailsForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      // Convert months to years for the form
      const loanTermYears = Math.max(1, Math.round(savedData.loanDuration / 12));
      
      // Pre-populate form with saved data
      setValue('loanAmount', savedData.loanAmount);
      setValue('loanTerm', loanTermYears);
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors } } = useForm<LoanDetailsFormData>({
    ...
    defaultValues: {
      ...
      loanTerm: formData.loanTerm || 5,
    }
  });
  
  const loanAmount = watch('loanAmount') || 10000;
  const loanTerm = watch('loanTerm') || 5;
  ...
  const chartData = [
    ...
    { name: 'Fees', value: calculatedValues.initiationFee + 
                          (calculatedValues.monthlyServiceFee * loanTerm * 12) + 
                          (calculatedValues.monthlyInsurance * loanTerm * 12), 
      color: '#22c55e' }
  ];
          ...
          <div className="flex flex-col space-y-4">
            <div className="p-4 bg-secondary/50 rounded-lg">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Based on {loanTerm} years term at {calculatedValues.interestRate}% APR
              </p>
            </div>
